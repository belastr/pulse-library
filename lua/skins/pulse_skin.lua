...
SKIN.PrintName		= "Pulse Library Skin"
SKIN.Author		    = "Sooth"
SKIN.DermaVersion	= 1
SKIN.GwenTexture	= Material("gwenskin/pulse.png")
...
SKIN.fontFrame					= "PulseDefaultBold"
...
SKIN.colTextEntryTextPlaceholder= Color(255, 255, 255, 225)
...
function SKIN:PaintTextEntry(panel, w, h)
	...
	if panel:IsEditing() or panel:GetParent().IsEditing and panel:GetParent():IsEditing() and panel:GetParent().TextArea == panel then
		textColor = color_black
	end

	panel:DrawTextEntryText(textColor, panel:GetHighlightColor(), panel:GetCursorColor())
end
...
function SKIN:PaintSliderKnob(panel, w, h)
	...
	if panel.Depressed or panel:GetParent():IsEditing() or panel:GetParent():GetParent():IsEditing() then
		return self.tex.Input.Slider.H.Down(0, 0, w, h)
	end

	if panel.Hovered or panel:GetParent():IsHovered() or panel:GetParent():GetParent():IsHovered() then
		return self.tex.Input.Slider.H.Normal(0, 0, w, h)
	end
...
function SKIN:PaintNumSlider(panel, w, h)
	...
	if panel:GetParent():IsEditing() then
		surface.SetDrawColor(0, 0, 0)
	elseif panel:GetParent():IsHovered() or IsValid(panel.Knob) and panel.Knob:IsHovered() then
...
function SKIN:PaintCategoryButton(panel, w, h)
	if panel.AltLine then
		...
		if not panel:IsEnabled() then skinColor = self.Colours.Category.LineAlt.Button_Disabled
		...
		if not panel:IsEnabled() then skinColor = self.Colours.Category.Line.Button_Disabled
...
derma.DefineSkin("Pulse", "Pulse Library Skin", SKIN)